...
export function Button({ title, handleClick }) {
    return (
        <button className="flex jusitfy-start items-center w-full  space-x-6 focus:outline-none text-white focus:text-indigo-400   rounded pb-4" onClick={handleClick}>
        ...
        <div className="h-screen flex">
            <div className="rounded-r bg-gray-900 xl:hidden flex justify-between w-full p-6 items-center ">
                <div className="flex justify-between  items-center space-x-3">
                    ...
                    <p className="text-2xl leading-6 text-white">PTT TAS</p>
                </div>
            ...
            <div id="Main" className={`${show ? 'translate-x-0' : '-translate-x-full'} xl:rounded-r transform  xl:translate-x-0  ease-in-out transition duration-500 flex justify-start items-start h-full  w-full sm:w-64 bg-gray-900 flex-col`}>
                ...
                <div className="mt-6 flex flex-col justify-start items-center  pl-4 w-full border-gray-600 border-b space-y-3 pb-5 ">
                    {
                        buttons.map((el, i) => <Button title={el.title} handleClick={() => go(el.path)}></Button>)
                    }
                </div>
